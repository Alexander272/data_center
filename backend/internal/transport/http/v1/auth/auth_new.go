...
func (h *AuthHandlers) signIn(c *gin.Context) {
	...
	logger.Debug(h.auth.Domain)
	c.SetCookie(h.cookieName, user.RefreshToken, int(h.auth.RefreshTokenTTL.Seconds()), "/", h.auth.Domain, h.auth.Secure, true)
...
func (h *AuthHandlers) signOut(c *gin.Context) {
	...
	c.SetCookie(h.cookieName, "", -1, "/", h.auth.Domain, h.auth.Secure, true)
