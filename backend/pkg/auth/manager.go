...
type TokenManager interface {
	NewJWT(userId string, user interface{}, ttl time.Duration) (time.Time, string, error)
...
func (m *Manager) NewJWT(userId string, user interface{}, ttl time.Duration) (iat time.Time, token string, err error) {
	iat = time.Now()
